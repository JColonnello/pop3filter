...
int set_filter(ServerArguments *args, char *username, int *fd1)
{
	...
	int fd[2];
	int fd2[2];
	char pop3filter_version[] = "POP3FILTER_VERSION=" POP3_VERSION;
	if (username == NULL)
		username = "";
	char *pop3_username = calloc(sizeof("POP3_USERNAME=") + strlen(username), 1);
	strcat(pop3_username, "POP3_USERNAME=");
	strcat(pop3_username, username);
	char pop3_server[] = "POP3_SERVER=" POP3_SERVER;
	...
	{ //Child process
		...
		if (ret == -1)
		{
			log(LOG_ERROR, "execle() failed: %s", strerror(errno));
			return -1;
		}
	}
	else
